...
export default function AdminPage() {
    ...
    return (
      <div className="page-container max-w-2xl">
        <div className="flex items-center justify-between mb-4">
          <h1 className="section-title">{isNew ? 'Új recept' : 'Recept szerkesztése'}</h1>
        ...
        <div className="space-y-4 bg-card border rounded-lg p-5">
          <div>
            <label className="text-sm font-medium mb-1 block">Név</label>
            <Input value={editing.name} onChange={e => setEditing({ ...editing, name: e.target.value })} />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-sm font-medium mb-1 block">Kategória</label>
              <select
            ...
            <div>
              <label className="text-sm font-medium mb-1 block">Étkezés típus</label>
              <select
                value={editing.mealType}
                onChange={e => setEditing({ ...editing, mealType: e.target.value as MealType })}
                className="w-full bg-background border rounded-md px-3 py-2 text-sm"
              >
                {Object.entries(MEAL_TYPE_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
              </select>
            </div>
          </div>
          <div>
            <label className="text-sm font-medium mb-1 block">Alapértelmezett adag (fő)</label>
          ...
          {/* Ingredients */}
          <div>
            <label className="text-sm font-medium mb-2 block">Hozzávalók</label>
            <div className="space-y-2">
              {editing.ingredients.map((ing, i) => (
                <div key={i} className="flex gap-2 items-center">
                  <Input placeholder="Név" value={ing.name} onChange={e => updateIngredient(i, 'name', e.target.value)} className="flex-1" />
                  <Input placeholder="Menny." value={ing.quantity || ''} onChange={e => updateIngredient(i, 'quantity', e.target.value)} className="w-20" />
                  <Input placeholder="Egység" value={ing.unit} onChange={e => updateIngredient(i, 'unit', e.target.value)} className="w-20" />
  ...
  return (
    <div className="page-container max-w-3xl">
      <div className="flex items-center justify-between mb-4">
        <h1 className="section-title">Receptek kezelése</h1>
        <div className="flex gap-2">
          <Button onClick={startNew} className="gap-1"><Plus className="w-4 h-4" /> Új recept</Button>
          <Button variant="outline" onClick={() => confirm('Alapértelmezett receptek visszaállítása?') && resetToDefault()}>
            Visszaállítás
          </Button>
        </div>
      </div>

      <Input placeholder="Keresés..." value={filter} onChange={e => setFilter(e.target.value)} className="mb-4" />
        ...
        {filtered.map(recipe => (
          <div key={recipe.id} className="flex items-center justify-between px-4 py-3 hover:bg-secondary/30 transition-colors">
            <div>
              ...
              <span className="text-xs text-muted-foreground ml-2">{CATEGORY_LABELS[recipe.category]}</span>
            </div>
