...
function MobilePlan({ activeDay, setActiveDay, editing, setEditing, showWeekOverview, setShowWeekOverview, weekPlan, recipes, options, dinnerOptions, updateDay, changeMessage, canUndo, onUndo }: MobilePlanProps) {
  ...
  return <div className="md:hidden" data-testid="mobile-planner">
    ...
    <div className="mb-3 flex gap-2">
      ...
      <Button variant={editing ? 'default' : 'outline'} size="sm" className="flex-1 gap-1" onClick={() => { setShowWeekOverview(false); setEditing(!editing); }}><Pencil className="h-4 w-4" /> {editing ? tr('Szerkesztés kész', 'Done editing') : tr('Ételek és adagok szerkesztése', 'Edit meals and servings')}</Button>
    </div>
...
function CompactMeal({ label, recipe, displayName, servings, onReplace }: { label: string; recipe?: Recipe; displayName?: string; servings: number; onReplace: () => void }) {
  ...
  return <div className="flex items-center gap-3 rounded-xl border bg-secondary/20 p-2.5">
    <Link to={`/recipes/${recipe.id}?from=plan&servings=${servings}`} state={{ from: '/planner/week' }} className="flex min-w-0 flex-1 items-center gap-3">
...
function MealSlot({ label, value, servings, days = 1, options, recipes, onChange, onServingsChange, onDaysChange }: { label: string; value: string | null; servings: number; days?: number; options: Recipe[]; recipes: Recipe[]; onChange: (value: string | null) => void; onServingsChange: (value: number) => void; onDaysChange?: (value: number) => void }) {
  ...
  return <div className="rounded-lg bg-secondary/35 p-3">
    ...
    {recipe && <div className="flex gap-3 rounded-lg border bg-card p-2.5">
      <Link to={`/recipes/${recipe.id}?from=plan&servings=${servings}`} state={{ from: '/planner/week' }} className="shrink-0 overflow-hidden rounded-md"><img src={recipe.imageUrl} alt={recipe.name} className="h-[70px] w-24 object-cover" /></Link>
      <div className="min-w-0 flex-1"><Link to={`/recipes/${recipe.id}?from=plan&servings=${servings}`} state={{ from: '/planner/week' }} className="line-clamp-2 text-sm font-semibold hover:text-primary">{recipe.name}</Link><Link to={`/recipes/${recipe.id}?from=plan&servings=${servings}`} state={{ from: '/planner/week' }} className="text-xs text-primary hover:underline">{tr('Recept megnyitása', 'Open recipe')}</Link>
