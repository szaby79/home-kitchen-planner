...
describe('dietary recipe pack five', () => {
  it('adds 12 complete recipes without exact name or ID duplicates', () => {
    expect(dietaryRecipePackFive).toHaveLength(12);
    ...
    dietaryRecipePackFive.forEach(recipe => {
  ...
  it('adds balanced vegan, vegetarian, and keto coverage', () => {
    expect(dietaryRecipePackFive.filter(recipe => recipe.vegan)).toHaveLength(5);
    expect(dietaryRecipePackFive.filter(recipe => recipe.vegetarian)).toHaveLength(7);
    expect(dietaryRecipePackFive.filter(recipe => recipe.keto)).toHaveLength(10);
    expect(dietaryRecipePackFive.filter(recipe => recipe.keto && recipe.vegan)).toHaveLength(4);
  });

  it.each<FoodRestriction>(['milk', 'egg', 'nuts', 'fish', 'soy'])(
    'blocks every pack-five recipe carrying the %s allergen',
    restriction => {
      const affected = dietaryRecipePackFive.filter(recipe => recipeAllergens(recipe).has(restriction));
  ...
  it('keeps several keto meals available under all supported allergen filters', () => {
    const safeKeto = dietaryRecipePackFive.filter(recipe => recipe.keto && recipeAllergens(recipe).size === 0);
    expect(safeKeto.length).toBeGreaterThanOrEqual(4);
    safeKeto.forEach(recipe => expect(recipeMatchesSafetyPreferences(recipe, {
      ...DEFAULT_MENU_PREFERENCES,
      allergies: ['gluten', 'milk', 'egg', 'nuts', 'fish', 'soy'],
      diet: 'keto',
    }), recipe.name).toBe(true));
  });
});
